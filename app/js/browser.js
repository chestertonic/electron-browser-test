...
onload = function() {
  ...
  // Test for presence of the experimental <webview> zoom and find API
...
function doLayout() {
  ...
  var tabHeight = tabContainer.offsetHeight;
...
function handleFindUpdate(event) {
  ...
  if (event.finalUpdate && !event.canceled) {
    ...
    if (findBoxObscuresActiveMatch(findBoxRect, event.selectionRect)) {
      // Move find box out of way if there is room
      // or make semi-transparent
...
function handleLoadStop(event) {
  // Let spinner animation finish before
  // loading class for smoother transition.
  isLoading = false;
}
